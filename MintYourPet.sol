...
contract MintYourPet is TNT721 {

    address public admin; //me
    ...
    uint256 public nftPrice = 200000000000000000000; // 200 Tfuel
    ...
    function mintTo(string memory link) public payable{
        require(msg.value == nftPrice, "Not enough TFuel Sent");

        uint mintIndex = totalSupply();
        _safeMint(msg.sender, mintIndex);
        _setTokenURI(mintIndex, link);
    }

    //emergency Function to claim TFuel, should be sent automatically
    ...
    function setNFTURI(uint256 tokenMintIndex, string memory link) public {
        require(msg.sender == admin, "You are not allowed to do that.");
        _setTokenURI(tokenMintIndex, link);
    }
}